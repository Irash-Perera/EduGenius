...
def QA_RAG(query,llm,vs1_path,vs2_path,k,threshold,session = None):
    ...
    if session != None:
        ...
        hints = session["hints"]
        marking_scheme = session["marking_scheme"]
        ...
        if marking_scheme != None:
            marking_scheme_str = f"marking scheme : {reformat_curly_brackets(str(marking_scheme))}"
    ...
    Student has tried a question and all the data related to that question has been provided such as question, correct marking scheme, answer, explanation, marks, similar problems, improvements, hints and history. These are the data that has been provided to you to generate the answer.\n"""+'Original question: '+question_str+'\n'+'Correct original marking scheme: '+marking_scheme_str+'\n'+"Student provided answer: "+answer_str +'\n'+"Given marks by the system:"+str(marks) +'\n'+"Explanation given by the system: "+explanations_str +'\n'+"Allocated marks for the provided answer: "+marks_str +'\n'+"Improvements can be done to student's answer: "+improvements_str +'\n'+"Previous chat history: "+history  +'\n'+"Similar problems to the problem student has just tried: "+similar_problems_str +'\n'+"Hints generated for the student: "+hints_str+'\n'+"""
    If the student is asking more questions go beyond the above context and explain and answer with your knowledge. If the student asks beyond the mathematics, you should kindly ask him to ask only math-related questions. You can use the textbook content to generate the answer if it is relevant to the question. If in the correct marking scheme section, the marks allocated for each step is given, you can show it as the marking scheme if the user asked how the marks are given. Answer in friendly, explaining manner.If the student asks to generate mathematical steps, always try to go with  the steps provided in the explanation and the correct marking scheme. If you can see more relevant information in the textbook content, you can use that information to generate the answer.Ignore the context section. Strictly follow the given correct marking scheme to allocate the marks. Maximum marks can be granted is 2.
    ...
    Answer the user query based on the following instructions and data provided.
    Always give the response in correct markdown format. for the mathematical equations and the signs always use the 100 percent correct mathml format, but never use latex. 
    ...
    If you are asked to generate mathematical steps, always try to go with the steps provided as the potential answer explaining each step more. Always remeber, the highest marks can student get is 2 for each question. Reffer the context to see how marks are allocated for each step.
    You can use the textbook content to generate the answer if it is relevant to the question. 
    If the user asks beyond the mathematics, you should kindly ask him to ask only math-related questions.If the context does not have the information about the question student asks, you can answer with your knowledge.
    Follow the above steps thouroughly and answer in friendly, explaining manner. Complex mathematical phrases shouldbe shown in only mathml format such as fractions, square roots, matrices, etc. 
    textbook content:"""+ relevent_textbook_content +"""
    ...
    # print('-----------------------------------------------------------------------')
    # print(str(wolfram_text))
    # print('-----------------------------------------------------------------------')
    # print(str(relevent_textbook_content))
    # print("=======================================================================")
    
    return response,wolfram_text,relevent_textbook_content,template
...
def respond_for_user_question(user_question,llm):
    ...
    response,_,__,___ = QA_RAG(user_question, llm,"vectorstore_text_books","vectorstore_2018_OL", 2, 0.4, dict(st.session_state))
